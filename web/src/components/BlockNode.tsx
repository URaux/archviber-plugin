...
import type { BlockNodeData, BuildStatus } from '../types'

const statusDotClasses: Record<BuildStatus, string> = {
  idle:     'bg-slate-300',
...
export function BlockNode({ data, selected }: NodeProps) {
  ...
  return (
    ...
    >
      ...
      <Handle id="t-left"   type="target" position={Position.Left}   className={handleClasses} style={{ top: '50%' }} />
      <Handle id="t-right"  type="target" position={Position.Right}  className={handleClasses} style={{ top: '50%' }} />
      ...
      <div className="pr-5">
        ...
        <div className="mt-1 text-[11px] leading-4 text-slate-400">
          {blockData.description || 'Double-click to edit'}
        </div>
        {blockData.techStack ? (
      ...
      <Handle id="s-left"   type="source" position={Position.Left}   className={handleClasses} style={{ top: '50%' }} />
      <Handle id="s-right"  type="source" position={Position.Right}  className={handleClasses} style={{ top: '50%' }} />
    </div>
  )
}
