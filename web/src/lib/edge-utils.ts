 ...
 * Blocks inside the same container are laid out with ELK, so we compare relative positions.
 * Cross-container connections use absolute position to determine the best direction.
 */
export function assignHandles(
  ...
  if (sameContainer) {
    ...
    // Pick the axis with the larger delta — vertical chains use top/bottom
