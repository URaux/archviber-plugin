/**
 * ELK.js layout engine — ported from ArchViber.
 */
import ELK, { type ElkExtendedEdge, type ElkNode } from 'elkjs/lib/elk.bundled.js'
import type { Edge, Node } from '@xyflow/react'
import type { CanvasNodeData, ContainerNodeData } from '../types'
import { assignAllEdgeHandles } from './edge-utils'
...
export async function layoutArchitectureCanvas(
    ...
    return {
      ...
      height: containerData.collapsed
        ...
        : Math.max(
            ...
            typeof container.style?.height === 'number' ? container.style.height : CONTAINER_MIN_HEIGHT
          ),
      layoutOptions: {
      ...
      children: containerData.collapsed
        ? []
  ...
  for (const child of layout.children ?? []) {
    if (containerIds.has(child.id)) {
      positionMap.set(child.id, { x: child.x ?? 0, y: child.y ?? 0, width: child.width, height: child.height })
      for (const block of child.children ?? []) {
        positionMap.set(block.id, { x: block.x ?? 0, y: block.y ?? 0 })
      }
      continue
    }
  ...
  return {
    ...
    edges: assignAllEdgeHandles(
      ...
      clonedEdges.map((edge) => {
        ...
        return { ...edge, data: { ...(edge.data ?? {}), isIntraContainer } }
      })
    ),
  }
}
